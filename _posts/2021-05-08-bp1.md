...
We see that, when (h sub sigma = 0), $\mathbf{p}_{\sigma}$ only decreases. This happens when $\mathbf{p}_{\sigma}$ is too large, so it allows some 'negative feedback' on the adaptation of $\Sigma$.
(Sorry for the notation change on 'h sub sigma', for some reason, Github markdown breaks down when I use Latex formatting there).
