...
const createMarkdownFile = async () => {
    // Prompt for title
    const title = await promptUser('Enter the blog post title (use hyphens for spaces): ');
    if (!title) {
    ...
    // Prompt for tags
    const tags = await promptUser('Enter comma-separated tags (leave blank for none): ');
    ...
    const permalink = `/posts/${encodeURIComponent(fileName.replace(/ /g, '-').replace('.md', ''))}`;
    const tagsFormatted = formatTags(tags);
